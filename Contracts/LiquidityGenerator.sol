...
contract LiquidityGenerator {
    ...
    event Finalized(uint amountGRM, uint amountETH);
    event Deposit(
    ...
    function finalize() public {
        ...
        uint _amountGRM = gremlin.myBalance();
        uint _amountETH = address(this).balance;

        uint _amountGRM1 = _amountGRM.mul(sharesRouter1).div(totalRouterShares);
        uint _amountETH1 = _amountETH.mul(sharesRouter1).div(totalRouterShares);
        uint _amountGRM0 = _amountGRM.sub(_amountGRM1);
        uint _amountETH0 = _amountETH.sub(_amountETH1);

        gremlin.safeApprove(router0, _amountGRM0);
        gremlin.safeApprove(router1, _amountGRM1);
        ...
        finalized = true;
        emit Finalized(_amountGRM, _amountETH);
    }
    ...
    function deposit() external payable {
        ...
        require(msg.value >= 15e18, "LiquidityGenerator: INVALID_VALUE");
